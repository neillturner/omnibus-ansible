...
if [ ! "$(which ansible-playbook)" ]; then
  if [ -f /etc/centos-release ] || [ -f /etc/redhat-release ] || [ -f /etc/oracle-release ] || [ -f /etc/system-release ]; then
    echo "------> centos, redhat, oracle, or amazon: installing with yum"
    ...
    yum -y install python-devel MySQL-python sshpass libffi-devel openssl-devel && pip install pyrax pysphere boto passlib dnspython  pyopenssl
    ...
    echo "------> debian or ubuntu: installing with apt"
    wait_for_cloud_init
    dpkg_check_lock && apt-get update -q

    echo "------> Install required Python libs and pip"
    ...
    echo "------> If python-pip install failed and setuptools exists, try that"
    if [ -z "$(which pip)" ] && [ -z "$(which easy_install)" ]; then
    ...
    echo "------> Install upgraded setuptools if ubuntu trusty"
    [ X`lsb_release -c | grep trusty | wc -l` = X1 ] && pip install -U setuptools==39.0.1
    echo "------> Install cryptography if ubuntu trusty"
